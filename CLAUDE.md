...
SELECT 만 갖는다(권한으로 강제). 수정은 `erflow_mig` 에만.

**커밋 전 `./gradlew build`.** Checkstyle 위반은 빌드 실패다. 테스트 348건 중
273건은 DB 를 상대로 하므로 `application-local.yml` 이 있어야 돈다.
...
| **비밀번호 해시** | 레거시 알고리즘을 바꾸면 기존 사용자 전원이 로그인 못 한다. 바꾸려면 로그인 시 재해시로 점진 교체(O-006) |
...
2. **화면 재설계급** — D-077(근무 통계 한 줄), D-068(라벨 통일), D-069(그래프)는
   디자인 변경 때 함께
3. 나머지 미결 — O-002·003·004(데드 레코드·오타·용어), O-006(해시), O-007(비밀번호=사번),
   O-009(고아 첨부), O-011(메일 발송), O-013(DeleteProcess 프로시저)
